...
import com.eharmony.aloha.io._
...
import com.eharmony.aloha.io.multiple.{ SequenceMultipleReadable, MultipleAlohaReadable }
...
case class TypedModelFactory[A: RefInfo, B: RefInfo: JsonReader: ScoreConverter](factory: ModelFactory, semantics: Option[Semantics[A]] = None)
   ...
   */
  val modelAndInfo =
    new ReadableByString[Try[ModelInfo[Model[A, B]]]] with GZippedReadable[Try[ModelInfo[Model[A, B]]]] with MultipleAlohaReadable[Try[ModelInfo[Model[A, B]]]] with SequenceMultipleReadable[ReadableSource, Try, ModelInfo[Model[A, B]]] with LocationLoggingReadable[Try[ModelInfo[Model[A, B]]]] with Logging {
