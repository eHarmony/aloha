...
object SchemaBasedTokenizer {
  ...
  /**
    * A parser combinator-based tokenizer for use in conjunction with the a schema based protocol.  This
    ...
    * @param rightBracket
    */
  private[tokenization] case class Tokenizer(leftBracket: String, rightBracket: String) extends JavaTokenParsers {

    /**
      * Create a parser that consumes an entire string and returns a list of Token instances that contains a
      * specification for an accessor function.  Note, this accessor may not exist in a given schema
