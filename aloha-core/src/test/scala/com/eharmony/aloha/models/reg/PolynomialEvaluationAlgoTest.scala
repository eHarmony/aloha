...
class PolynomialEvaluationAlgoTest {
    ...
    private[this] val semantics = new Semantics[Map[String, String]] {
        def close() {}
        def refInfoA = RefInfo[Map[String, String]]
    ...
    private[this] val factory = ModelFactory(RegressionModel.parser).toTypedFactory[Map[String, String], Double](semantics)
    ...
    @Test def testJsonParsedPolyEval() {
        ...
        val score = m.score(x)
        assertEquals(expected, score.relaxed.asDouble.get, tolerance)
    }
}
