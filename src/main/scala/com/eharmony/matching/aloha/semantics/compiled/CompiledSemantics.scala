...
case class CompiledSemantics[A](
    ...
    /** Non-blocking thread safe cache.
      */
    private[this] val cache = new NoEvictionCache()
...
object CompiledSemantics {
    ...
    /** This won't be necessary once we can return a future rather than waiting on the future before returning.
      */
    private val maxGenWaitInSec = 120
    private val generatedAccessorClassName = GeneratedAccessor.getClass.getCanonicalName.replaceAll("""\$$""", "")
    private val genFuncClassName = GenFunc.getClass.getCanonicalName.replaceAll("""\$$""", "")
}
