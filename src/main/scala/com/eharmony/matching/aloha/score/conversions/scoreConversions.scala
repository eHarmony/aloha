...
/** This object DOES NOT differentiate between a missing score and a score that could not be retrieved because the
  ...
  * Example usage:
  * {{{
  * import com.eharmony.matching.aloha.score.conversions.RelaxedOps._
...
/** This object differentiates between a missing score and a score that could not be retrieved because the wrong type
  ...
  * [[com.eharmony.matching.aloha.score.conversions.RelaxedConversions]] but encodes additional information.
  * Example usage:
  * {{{
  * // Scala
  * import com.eharmony.matching.aloha.io.score.ops.StrictConversions._
  * val asdf: Option[Boolean] = asBoolean(s).map(_.fold(t => throw new Exception("Tried to get a " + t + " as a Boolean"), identity))
  *
  * // Java
  * import static com.eharmony.matching.aloha.score.conversions.StrictConversions.*;
  * // ...
  * Boolean b = null;
  * try {
  *   b = asJavaBoolean(s);
  ...
  * will throw an  [[com.eharmony.matching.aloha.score.conversions.IncorrectScoreTypeAccessException]].
  */
object StrictConversions extends BasicTypeScoreConversions[({type L[+A]=Option[Either[ScoreType, A]]})#L] {
    ...
    /** If no score is present, return None; if a score is present but the wrong type of score was queried, throw an
      * [[com.eharmony.matching.aloha.score.conversions.IncorrectScoreTypeAccessException]].
      * @param s
      * @param container
      * @tparam A
      * @return
