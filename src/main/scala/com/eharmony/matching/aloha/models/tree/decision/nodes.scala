...
/** A simple representation of a decision tree node.  It is just like a [[com.eharmony.matching.aloha.models.tree.Tree]]
...
sealed trait Node[-A, +B] extends Tree[B, immutable.IndexedSeq, Node[A, B]] {
    ...
    /** Find the deepest node in the tree possible, given the data (in v), on which to branch.  Progress down the
      ...
      *
      * [[scala.util.Right]][[com.eharmony.matching.aloha.models.tree.decision.Leaf]] when a leaf is reached or
      * [[scala.util.Right]][[(com.eharmony.matching.aloha.models.tree.decision.InteriorNode, Option[String])]]
      ...
      * @return either a [[scala.util.Right]][[com.eharmony.matching.aloha.models.tree.decision.Leaf]] representing a
    ...
    private[decision] def getNode[A, B](n: Node[A, B], v: A): Either[InteriorNodeResult[A, B], Leaf[B]] = n match {
        ...
        case interior: InteriorNode[A, B] => interior.nodeSelector(v) match {
            ...
            case i if i > n.descendants.size =>
                Left(InteriorNodeResult(interior, Seq("childSelector returned i=" + i + ". Size = " + n.descendants.size), Nil))
...
case class Leaf[+B](value: B) extends Node[Any, B] {
    ...
    /** Return this node wrapped in a [[scala.util.Right]].
      ...
      * @return either a [[scala.util.Right]][[com.eharmony.matching.aloha.models.tree.decision.Leaf]] this node.
      */
