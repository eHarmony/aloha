...
public class NonSpringJavaTest {
    ...
    static {
        try {
            ...
            VFS2_MODEL_LOCATION_URL_STRING = props.getProperty("model_location_1");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * A test showing how to instantiate the factory and model directly from Java (instead of Spring).  Note that the
     * model code is not really idiomatic because it copies the control flow
     * @throws FileSystemException
     */
    @Test
    public void test1()
            throws FileSystemException // b/c VFS.getManager();
    {
    ...
    /**
     * This is the code actually used to test the correctness of the model.  Same as in the SpringModelFactoryTest.
    ...
    private static void testModel(Model<TestProto, Double> model) {
        ...
        assertEquals(SpringModelFactoryTest.EXPECTED_1, d, SpringModelFactoryTest.TOLERANCE);
    }

    /**
     * Construct a model given a factory.  This code isn't exactly production code but should be good for
     * illustrative purposes.
     * @param modelFactory a factory
     * @param fo2Model a file from which to construct a model
    ...
    private static Model<TestProto, Double> getModel(
        ...
        if (modelTry.isSuccess()) {
            ...
            // This will definitely throw (b/c isSuccess() == false) but JVM doesn't know because it is unchecked.
            // For more information, see API:  http://www.scala-lang.org/api/2.10.1/index.html#scala.util.Try
            try {
                return modelTry.get();
            }
            catch(Exception e) {
                throw new RuntimeException("Rethrowing exception for kicks (SHOULDN'T NECESSARILY DO THIS IN PROD)", e);
            }
        }
    }
     ...
     *
     * Notice there are some casting issues because the code was written to be useful in Scala and the code employs
    ...
    private static TypedModelFactory<TestProto, Double> getModelFactory() {
        ...
        // ------------------------------------------------------------------------------------------------
        //  Construct the proto plugin instance that will handle protocol buffer based input
        // ------------------------------------------------------------------------------------------------
        final CompiledSemanticsProtoPlugin<TestProto> userPairingProtoPlugin =
                new CompiledSemanticsProtoPlugin<TestProto>(TestProto.class);

        // ------------------------------------------------------------------------------------------------
        //  Construct the compiler instance responsible for compiling the synthesized model features.
        //
        //  NOTE: In production, it is important to specify a class cache directory if the model or
        //        any of it's features are intended to be used more than one time.  This won't help if
        //        the features have never been compiled; however, if they have, a class cache directory
        //        will make the compilation process many times faster because the JVM class files don't
        //        need to be recompiled.
        // ------------------------------------------------------------------------------------------------
        final TwitterEvalCompiler compiler = new TwitterEvalCompiler(new File(CLASS_CACHE_DIR));
        ...
        //  We want to convert it to a typed factory by providing semantics, and reflection information
        //  about the data types that the models consume and produce.
        // ================================================================================================
        final ModelFactory untypedModelFactory = new ModelFactory(parsers);

        // ================================================================================================
        //  Create the typed factory.  This factory produces models that take TestProto instances as
        //  input and produces ieee-754 64-bit floats as output.
        //
        //  In general, calling the score(..) function that returns a proto is going to be the most
    ...
    private static Properties getTestProps() {
        ...
        try {
            props.load(vfs2FileManager.resolveFile("res:mvn_gen_test.properties").getContent().getInputStream());
            props.load(vfs2FileManager.resolveFile("res:spring_test/spring_test.properties").getContent().getInputStream());
            return props;
        }
        catch (IOException e) {
            throw new RuntimeException(e);
