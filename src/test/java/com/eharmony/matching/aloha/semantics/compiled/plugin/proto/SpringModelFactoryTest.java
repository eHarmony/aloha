...
public class SpringModelFactoryTest {
    ...
    /**
     * Expected value of running CompiledSemanticsProtoPluginTest.getProto(1) through the model.
     *
     * 0.12421 = 0.100000 +
     *           0.020000 +
     *           0.004000 +
     ...
     *
     * Features:
     *   intercept -> List((intercept,1.0))
     *   num_c3s -> List((num_c3s=0,1.0))
     *   required_int_value -> List((required_int_value=1,1.0))
     *   optional_defaulted -> List((optional_defaulted=1,1.0))
     *   distinct_chars -> List((distinct_chars=3,1.0))
     */
    ...
    /**
     * IMPORTANT: Needs to be referred to by the interface AlohaReadable rather than the actual class name.  This is
     * because java doesn't play nice with higher kinds.  See http://en.wikipedia.org/wiki/Kind_(type_theory) for more
    ...
    public void testUsingModelPulledFromSpringCtx() {
        ...
        final Double d1 = StrictConversions.asJavaDouble(models.get(1).score(p));
        assertEquals(EXPECTED_1, d1, TOLERANCE);
    }

    /**
     * Load a bunch of TestProto instances.  This is stupid because they are the same as the ones in
     * CompiledSemanticsProtoPluginTest.protos but b/c of compile order issues in maven, those are not present at
     * the time this test is run.  So, they were serialized and stuck in the src/test/resources directory.
    ...
    private static List<TestProto> getTestExamples() {
        ...
        try {
            // Sloppy but whatever.
            for (int i = 0; i <= 7; ++i) {
                final InputStream is = VFS.getManager().resolveFile("res:spring_test/pb_instances/level_" + i + ".pb").getContent().getInputStream();
