...
//        CompiledSemantics(TwitterEvalCompiler(classCacheDir = Option(FileLocations.testGeneratedClasses)), plugin, Nil)
        CompiledSemantics(TwitterEvalCompiler(classCacheDir = Option(FileLocations.testGeneratedClasses)), plugin, Seq("com.eharmony.matching.aloha.feature.BasicFunctions._"))
    }
    ...
    @Test def test_BagOfWords2() {
        ...
        val f = ModelFactory(RegressionModel.parser).toTypedFactory[TestProto, Double](semantics)
