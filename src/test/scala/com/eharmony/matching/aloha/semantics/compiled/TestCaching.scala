...
class TestCaching {
    @Test def test() {
        ...
        println(s"$t1 seconds.  result: ${r1.foldLeft(0L)(_ + _.n)}. \n${r1.map(_.n).sorted}")
    }
    ...
    def run(eval: TwitterEvalCompiler, n: Int, timeout: Int, threads: Int) = {
        ...
        val r = new Random(0)
        val exp = Seq.fill(n)(r.nextInt(n / 2) + 1).map(i => s"""new com.eharmony.matching.aloha.semantics.compiled.Delegate((_:Any) => $i, "(_:Any) => $i", $i) """)
