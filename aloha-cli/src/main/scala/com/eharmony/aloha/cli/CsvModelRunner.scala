...
import com.eharmony.aloha.cli.InputPosition.{After, Before, Both, InputPosition, Neither}
...
import com.eharmony.aloha.semantics.compiled.plugin.csv.{CompiledSemanticsCsvPlugin, CsvLine, CsvLines, CsvProtocol, CsvTypes, Enum}
...
case class InlineCsvInputType(
    ...
    private[cli] def dupKeys[A, B](it: Iterable[(A, B)]) = it.groupBy(_._1).collect{ case (k, v) if 1 < v.size => k }
}


private[cli] sealed trait EnumGen {
    def withClassName(canonicalClassName: String): Enum
}

private[cli] case class WithNums(constants: Seq[(String, Int)]) extends EnumGen {
    def withClassName(canonicalClassName: String) = Enum(canonicalClassName, constants:_*)
}

private[cli] case class NoNums(constants: Seq[String]) extends EnumGen {
...
/** An enum parser.
  *
  */
private[cli] object EnumParser extends RegexParsers {
